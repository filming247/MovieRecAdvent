...
@app.post("/submit")
async def movie_results(request: Request) -> list:
    """Given queries, returns a list of movie results."""
    genre_dict = {
        "Action"    : 28,
        "Adventure" : 12,
        "Animation" : 16,
    ...
    # data = '{"with_genres"            : ["Action", "Animation"],\
    #                "with_runtime.lte"       : 90,\
    ...
    # dictionary = {'with_genres'            : ['Action', 'Animation'],
    #                'with_runtime.lte'       : 90,
    #                'primary_release_date'   : '2020-01-01',
    #                'with_watch_providers'   : ['Netflix', 'Disney+'],
    #                'num_results'            : 10}
    
    dictionary['with_genres'] = str([genre_dict[x] for x in dictionary['with_genres']])[1:-1].replace(' ', '')
